function [net, info] = cnn_xor(varargin)
...
opts.expDir = fullfile(vl_rootnn, 'data', ['xor-baseline2-' num2str(opts.mnistVer) sfx '_jitter_' num2str(opts.dataJitter) '_layers_num_' num2str(opts.layers_num) '_final_fc_' num2str(opts.final_fc) '_nl_' opts.non_linearity]) ;
...
if isempty(opts.network)
   ...
   net = cnn_xor_simple('batchNormalization', opts.batchNormalization, ...
...
data_x = 2*zeros([1,opts.ntrain+opts.ntest]);%-0.5;
data_y = 2*zeros([1,opts.ntrain+opts.ntest]);%-0.5;

classes = randi(4,1,opts.ntrain+opts.ntest);
...
data_x(idx1) = data_x(idx1) + 1;
data_y(idx1) = data_y(idx1) + 1;

data_x(idx2) = data_x(idx2) + 1;
data_y(idx2) = data_y(idx2) - 1;

data_x(idx3) = data_x(idx3) - 1;
data_y(idx3) = data_y(idx3) - 1;

data_x(idx4) = data_x(idx4) - 1;
data_y(idx4) = data_y(idx4) + 1;

classes(idx3) = 1;
classes(idx4) = 2;

idx_class1 = find(classes==1);
idx_class2 = find(classes==2);

figure;plot(data_x(idx1),data_y(idx1),'r.');hold on; 

plot(data_x(idx2),data_y(idx2),'g.');hold on; 
plot(data_x(idx3),data_y(idx3),'b.');hold on; 
plot(data_x(idx4),data_y(idx4),'c.');

data_out = data_x .* u1 + data_y .* u2;

data_out = [data_x;data_y];


set = [ones(1,opts.ntrain) 3*ones(1,opts.ntest)];

data_out=shiftdim(data_out,-2);

imdb.images.data = single(data_out) ;
